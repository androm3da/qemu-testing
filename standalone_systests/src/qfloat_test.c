...
static __attribute__((always_inline)) HVX_Vector create_hfv_from_sf(float value)
{
    __fp16 hf = value;
    union ui16f16 cvt;
    cvt.f16 = hf;
...
static __attribute__((always_inline)) HVX_Vector create_qf16v_from_sf(float value)
{
    /* create qf16 vector from hf */
    HVX_Vector tmp = Q6_Vqf16_vadd_Vqf16Vhf(Q6_V_vsplat_R(0), create_hfv_from_sf(value));
    return tmp;
}
...
static __attribute__((always_inline)) HVX_Vector convert_qf16v_to_hfv(HVX_Vector vect)
{
...
static __attribute__((always_inline)) float get_flt0_from_halfv(HVX_Vector vect)
{
    union ui16f16 cvt;
    cvt.i = (vect[0] & 0xffff);
...
static __attribute__((always_inline)) float get_flt0_from_qf16v(HVX_Vector vect)
{
    return get_flt0_from_halfv(convert_qf16v_to_hfv(vect));
}

/* get lowest float from a vector pair of qf32's */
static __attribute__((always_inline)) float get_flt0_from_qf32vp(HVX_VectorPair vect)
{
    union ui32f cvt;
...
int main(int argc, char **argv)
{
    ...
    printf("The sum of qf16 %.3f and hf   %.3f is %.3f\n", get_flt0_from_qf16v(v7),
           get_flt0_from_halfv(v6), get_flt0_from_qf16v(result11));
    ...
    printf("The sum of hf   %.3f and hf   -%.3f is %.3f\n", get_flt0_from_halfv(v5),
           get_flt0_from_halfv(v6), get_flt0_from_qf16v(result12));

    /* add the qf16 vectors into a qf16 vector */
    HVX_Vector result13 = Q6_Vqf16_vsub_Vqf16Vqf16(v7, v8);
    printf("The sum of qf16 %.3f and qf16 -%.3f is %.3f\n", get_flt0_from_qf16v(v7),
           get_flt0_from_qf16v(v8), get_flt0_from_qf16v(result13));


    /* add a qf16 vector and an IEEE hf vector into a qf16 vector */
    HVX_Vector result14 = Q6_Vqf16_vsub_Vqf16Vhf(v7, v6);
    printf("The sum of qf16 %.3f and hf   -%.3f is %.3f\n", get_flt0_from_qf16v(v7),
           get_flt0_from_halfv(v6), get_flt0_from_qf16v(result14));
    ...
    printf("The result of hf   %.3f * hf   %.3f is %.3f\n", get_flt0_from_halfv(v5),
           get_flt0_from_halfv(v6), get_flt0_from_qf16v(result15));
    ...
    printf("The result of qf16 %.3f * qf16 %.3f is %.3f\n", get_flt0_from_qf16v(v7),
           get_flt0_from_qf16v(v8), get_flt0_from_qf16v(result16));
    ...
    printf("The result of qf16 %.3f * hf   %.3f is %.3f\n", get_flt0_from_qf16v(v7),
           get_flt0_from_halfv(v6), get_flt0_from_qf16v(result17));
